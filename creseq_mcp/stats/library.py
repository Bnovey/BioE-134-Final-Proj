...
def _bh_fdr(pvals: pd.Series) -> pd.Series:
    """
    Benjamini-Hochberg FDR correction implemented without scipy/statsmodels.
    """
    p = pvals.astype(float).fillna(1.0).clip(0, 1).to_numpy()
    n = len(p)
    order = np.argsort(p)
    ranked = p[order]
...
def normalize_activity(
    ...
    Expected input columns:
        element_id, dna_counts, rna_counts

    If the file uses oligo_id instead of element_id, pass element_col="oligo_id".
    """

    df = _read_table(count_table_path)
    _check_cols(df, {element_col, dna_col, rna_col}, str(count_table_path))

    result = df.copy()
    result[dna_col] = pd.to_numeric(result[dna_col], errors="coerce").fillna(0)
    result[rna_col] = pd.to_numeric(result[rna_col], errors="coerce").fillna(0)

    result["log2_activity"] = np.log2(
        (result[rna_col] + pseudocount) / (result[dna_col] + pseudocount)
    )

    result["low_dna_coverage"] = result[dna_col] < 5
    result["low_rna_coverage"] = result[rna_col] < 5

    summary = {
        "n_elements": int(len(result)),
        "median_log2_activity": float(result["log2_activity"].median()),
        "mean_log2_activity": float(result["log2_activity"].mean()),
        "n_low_dna_coverage": int(result["low_dna_coverage"].sum()),
        "n_low_rna_coverage": int(result["low_rna_coverage"].sum()),
        "warnings": [],
        "pass": bool((result[dna_col] >= 5).mean() >= 0.80),
    }

    if not summary["pass"]:
        summary["warnings"].append(
            "More than 20% of elements have DNA counts < 5; activity estimates may be unstable."
        )
...
def call_active_elements(
    ...
    """
    Classify active CREs using an empirical negative-control background.

    Expected input:
        A table with log2_activity.

    Optional:
        designed_category column with negative_control rows.

    Interpretation:
        Active = log2_activity > threshold and q_value < fdr_threshold.
    """
    ...
    result = df.copy()
    result[activity_col] = pd.to_numeric(result[activity_col], errors="coerce")

    if category_col in result.columns and (result[category_col] == negative_control_label).any():
        null = result.loc[result[category_col] == negative_control_label, activity_col].dropna()
    else:
        # fallback: use bottom 20% as empirical inactive background
        null = result[activity_col].dropna()
        null = null[null <= null.quantile(0.20)]

    null_mean = float(null.mean())
    null_std = float(null.std(ddof=1)) if len(null) > 1 else 1.0
    if null_std == 0 or np.isnan(null_std):
        null_std = 1.0

    # Approximate one-sided empirical z-score p-value without scipy.
    # This is not a full GLM; it is a lightweight project-safe activity caller.
    z = (result[activity_col] - null_mean) / null_std
    result["activity_z"] = z

    # Normal-tail approximation using erf from numpy.
    # p = P(Z >= observed)
    result["p_value"] = 0.5 * (1 - np.vectorize(math.erf)(z / np.sqrt(2)))
    result["p_value"] = result["p_value"].clip(0, 1)
    result["q_value"] = _bh_fdr(result["p_value"])

    result["active"] = (
        (result[activity_col] > activity_threshold)
    ...
    summary = {
        ...
        "fraction_active": float(result["active"].mean()),
        "null_mean": null_mean,
        "null_std": null_std,
    ...
    if len(null) < 5:
        summary["warnings"].append(
            "Fewer than 5 negative/background controls were available; activity calls are lower confidence."
        )
...
# ---------------------------------------------------------------------------
# Optional raw FASTQ helper
# ---------------------------------------------------------------------------

def count_barcodes_from_fastq(
    fastq_path: str | Path,
    barcode_start: int = 0,
    ...
    This is the simple "start from raw FASTQ" helper.

    Assumption:
        The barcode is at a fixed position in each read.
    ...
    if not path.exists():
        raise ValueError(f"FASTQ path does not exist: {path}")
    ...
    with opener(path, "rt") as handle:
        while True:
            ...
            barcode = seq[barcode_start:end]
            if set(barcode) <= {"A", "C", "G", "T", "N"}:
    ...
    result = pd.DataFrame(
        [{"barcode": bc, "count": c} for bc, c in counts.items()]
    ).sort_values("count", ascending=False)

    summary = {
        "n_reads_processed": int(n_reads),
        "n_unique_barcodes": int(len(result)),
    ...
    if len(result) == 0:
        summary["warnings"].append("No barcodes were extracted. Check barcode_start/barcode_length.")

    return result, summary


def aggregate_fastq_counts_to_elements(
    dna_barcode_counts_path: str | Path,
    rna_barcode_counts_path: str | Path,
    barcode_map_path: str | Path,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    """
    Convert raw barcode count tables into element-level DNA/RNA counts.

    Inputs:
        dna_barcode_counts_path: table with barcode,count
        rna_barcode_counts_path: table with barcode,count
        barcode_map_path: table with barcode,oligo_id or barcode,element_id
    """

    dna = _read_table(dna_barcode_counts_path).rename(columns={"count": "dna_count"})
    rna = _read_table(rna_barcode_counts_path).rename(columns={"count": "rna_count"})
    bc_map = _read_table(barcode_map_path)

    _check_cols(dna, {"barcode", "dna_count"}, str(dna_barcode_counts_path))
    _check_cols(rna, {"barcode", "rna_count"}, str(rna_barcode_counts_path))
    _check_cols(bc_map, {"barcode"}, str(barcode_map_path))

    element_col = "element_id" if "element_id" in bc_map.columns else "oligo_id"
    _check_cols(bc_map, {element_col}, str(barcode_map_path))

    merged = bc_map.merge(dna, on="barcode", how="left").merge(rna, on="barcode", how="left")
    merged["dna_count"] = merged["dna_count"].fillna(0)
    merged["rna_count"] = merged["rna_count"].fillna(0)

    result = (
        merged.groupby(element_col)
        .agg(
            dna_counts=("dna_count", "sum"),
            rna_counts=("rna_count", "sum"),
            n_barcodes=("barcode", "nunique"),
        )
        .reset_index()
    )

    result["log2_activity"] = np.log2((result["rna_counts"] + 1) / (result["dna_counts"] + 1))

    summary = {
        "n_elements": int(len(result)),
        "n_barcodes_mapped": int(merged["barcode"].nunique()),
        "median_barcodes_per_element": float(result["n_barcodes"].median()),
        "warnings": [],
        "pass": True,
    }
